...
*   **본당/조직 자동 동기화 (Scraper)**: 
    ...
    *   **스크래퍼 보안 및 기술 특이사항**:
        *   **Anti-Bot 대응**: 교구 홈페이지 서버의 무한 리다이렉트(Redirect Loop)를 방지하기 위해 브라우저 수준의 Header(User-Agent, Accept-Language 등)와 쿠키 세션(`scratch/scraper_cookies.txt`)을 사용하도록 구현되었습니다.
        *   **가변 구조 파싱**: 지구 명칭 헤더가 `h3`, `h4`, `div` 등 다양하게 사용되는 점을 고려하여 `//h3[contains(text(), '지구')]`와 같은 유연한 XPath 쿼리를 적용했습니다.
        *   **UTF-8 보정**: `DOMDocument` 로드 시 한글 깨짐을 방지하기 위해 XML 인코딩 선언(`<?xml encoding="UTF-8">`)을 강제 삽입하여 처리합니다.
